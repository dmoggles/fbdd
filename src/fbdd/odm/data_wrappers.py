...
class Data:
    ...
    def violin(self, player1: str, player2: str, player1_year: int, player2_year: int, stats_to_use: List[DataAttribute], **violin_kwargs):
        if not self.data_unique_keys == [fc.PLAYER_ID, fc.YEAR]:
            raise ValueError(
                'Violin plot requires data with player and year as unique keys')
        return plot_violin(self, player1, player2, player1_year, player2_year, stats_to_use, **violin_kwargs)


class FbRefData(Data):
    def __init__(self, years: List[int]):
        dfs = [pd.read_parquet(
            f'https://kovadata.herokuapp.com/data/f/{year}') for year in years]
        data = pd.concat(dfs)
