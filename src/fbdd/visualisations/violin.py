...
VIOLIN_PLOT_SECTION_COLOURS = ["#0061FF",
                               "#4061D8", "#8061B0", "#BF6189", "#FF6161"]
IN_OPACITY = 1
OUT_OPACITY = 0.3
SMALL_SCALE = 0.75
...
def prep_data(data, player, year, stats_to_use):
    player_league = data.pipe(
        apply_filter, [Filter(fc.PLAYER, player, filters.EQ),
                       Filter(fc.YEAR, year, filters.EQ)]
    ).col(fc.COMPETITION).item()

    player_league_data = data.pipe(
        apply_filter, [Filter(fc.COMPETITION, player_league, filters.EQ), Filter(
            fc.YEAR, year, filters.EQ)]
    ).pipe(
        aggregate_by, [fc.PLAYER]
    )[stats_to_use]
...
def draw_half_violin(ax: plt.Axes, data, d, quantiles, stat, player_statline, kde, max_stat):
    ...
    for i in range(5):
        ...
        if player_statline >= min_v and player_statline < max_v:
            draw_filled_curve(ax, kde, min_v, player_statline,
                              d, VIOLIN_PLOT_SECTION_COLOURS[i], IN_OPACITY)
            draw_filled_curve(ax, kde, player_statline, max_v,
                              d, VIOLIN_PLOT_SECTION_COLOURS[i], OUT_OPACITY)
        else:
            if player_statline < min_v:
                opacity = OUT_OPACITY
            else:
                opacity = IN_OPACITY
            draw_filled_curve(ax, kde, min_v, max_v, d,
                              VIOLIN_PLOT_SECTION_COLOURS[i], opacity)


def plot_single_violin(ax, data, player1, player2, player1_year, player2_year, stat, player_highlight_colors):
    players = [player1, player2]
    years = [player1_year, player2_year]
    player_league_data = [prep_data(data, players[0], years[0], stat), prep_data(
        data, players[1], years[1], stat)]
    kdes = [stats.gaussian_kde(player_league_data[0].col(stat).fillna(
        0)), stats.gaussian_kde(player_league_data[1].col(stat).fillna(0))]
    quantiles = [get_quantiles(kdes[0], player_league_data[0], stat), get_quantiles(
        kdes[1], player_league_data[1], stat)]
    player_stat_line = [get_player_stat_line(player_league_data[0], players[0], stat), get_player_stat_line(
        player_league_data[1], players[1], stat)]
    max_cdfs = [kdes[0].integrate_box_1d(0, player_league_data[0].col(stat).max(
    )), kdes[0].integrate_box_1d(0, player_league_data[1].col(stat).max())]
    max_stat = max(player_league_data[0].col(
        stat).max(), player_league_data[1].col(stat).max())
    max_kde = max(estimate_maxima(player_league_data[0].col(
        stat)), estimate_maxima(player_league_data[1].col(stat)))
    for i, d in enumerate([1, -1]):
        draw_half_violin(
            ax, player_league_data[i], d, quantiles[i], stat, player_stat_line[i], kdes[i], max_stat)
    #ax.plot([0,max_stat], [0,0],  zorder=1)
    ax.hlines(0, 0, max_stat, color='black')

    player_ys = [kdes[0].evaluate(
        player_stat_line[0]), -kdes[1].evaluate(player_stat_line[1])]
    for i in range(2):
        ax.scatter([player_stat_line[i]], [player_ys[i]],
                   color=player_highlight_colors[i], zorder=2, s=60)
        ax.vlines(player_stat_line[i], 0, player_ys[i],
                  color=player_highlight_colors[i], zorder=2, linewidths=3)
        ...
        if i == 0:
            ...
            va = 'bottom'
        ax.text(max_stat*0.99, max_kde*1.18*(1-i*2), text,
                ha='right', va=va, color=player_highlight_colors[i])
...
def plot_violin(data: pd.DataFrame, player1: str, player2: str, player1_year: int, player2_year: int, stats_to_use: List[DataAttribute], player_highlight_colors: List[str] = ['darkgreen', 'olive'], small: bool = False, n_cols: int = 2) -> plt.Figure:
    """
    ...
    layout = np.concatenate([np.tile(['title'], (1, cols)),
                            main,
                             np.tile(['annot'], (1, cols))])

    fig, axes = plt.subplot_mosaic(layout,
                                   figsize=(5*cols*(SMALL_SCALE if small else 1), (1+2.5*rows)*(SMALL_SCALE if small else 1)), gridspec_kw={'height_ratios': [0.025]+[0.95/rows]*rows+[0.025]})
    axes_list = [v for k, v in axes.items() if k not in ['annot', 'title']]
    for ax, stat in zip(axes_list, stats_to_use):
        plot_single_violin(ax, data, player1, player2, player1_year,
                           player2_year, stat, player_highlight_colors)
    ...
    ax_text(
        0.5, 0.5, title,
        highlight_textprops=[{"color": player_highlight_colors[0]}, {
            "color": player_highlight_colors[1]}, ],
        size=16*(SMALL_SCALE if small else 1), fig=fig, textalign='center', ha='center', va='center', ax=axes['title']
    )

    ax_text(0.05, 0.5, "@ChicagoDmitry, data from fbref",
            size=10*(SMALL_SCALE if small else 1), fig=fig, textalign='center', ha='center', va='center', ax=axes['annot'], alpha=0.5)
    axes['title'].axis('off')
    axes['annot'].axis('off')
    fig.tight_layout()
    return fig
