...
def aggregate_by(data: pd.DataFrame, aggregate_cols: List[DataAttribute]) -> pd.DataFrame:
    ...
    transforms = {
        ...
        if c in DataAttribute._name_data_map and DataAttribute._name_data_map[c].agg_function
        else "sum"
        for c in data.columns
        if c not in DataAttribute._name_data_map or DataAttribute._name_data_map[c].agg_function
    }
...
def per_90(data: pd.DataFrame, rename: bool = False) -> pd.DataFrame:
    ...
    if rename:
        data_source = data_source.rename(
            columns={c: new_c for c, new_c in zip(columns_to_transform, __per_90_rename(columns_to_transform))}
        )
    return data_source
...
def rank(data: pd.DataFrame, pct: bool = True, rename: bool = False) -> pd.DataFrame:
    ...
    if rename:
        ranked = ranked.rename(columns={c: new_c for c, new_c in zip(ranked.columns, __rank_rename(ranked.columns))})
